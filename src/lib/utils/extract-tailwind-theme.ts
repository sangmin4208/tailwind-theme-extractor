...
type ThemeCategory = 'color' | 'text' | 'shadow' | 'radius' | 'font' | 'spacing' | 'breakpoint' | 'container' | 'tracking' | 'leading' | 'blur' | 'animation';
...
const parseVariables = (css: string, prefix: ThemeCategory): ThemeVariable[] => {
  ...
  const matches = [...css.matchAll(regex)].map(match => ({ name: match[1], value: match[2].trim() }));
